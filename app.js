...
const safeSeed={classes:[],courses:[],weekly:[],meetings:[],appointments:[],cases:[]};
let data=safeSeed;
...
function resourceBox(url,label){
  const ok=url&&/^https?:\/\//.test(String(url));
  return `<div style="flex:1;min-width:170px;border:1px solid #e5e9ee;border-radius:12px;padding:10px 12px;background:#fafbfc">
    <div style="font-size:12px;color:#7b8490;margin-bottom:8px">${esc(label)}</div>
    <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">${ok
      ? `<a class="button" href="${esc(url)}" target="_blank" rel="noopener">打开${esc(label)}</a><span class="badge good">已上传</span>`
      : `<span style="padding:8px 12px;border:1px solid #e5e9ee;border-radius:10px;background:#fff;color:#a0a8b2">打开${esc(label)}</span><span class="badge bad">未上传</span>`}
    </div>
  </div>`;
}
...
function render(){
  ...
  $('metrics').innerHTML=[['班级总数',classes.length||11],['当前需跟进',pending],['已录入课程',courses.length],['本周待办',(data.weekly||[]).filter(x=>pick(x,'状态','status')!=='已完成').length]].map(([l,n])=>`<div class="metric"><div class="label">${l}</div><div class="num">${n}</div></div>`).join('');
  const homeRows=classes.map(c=>`<tr><td><strong>${esc(c.name)}</strong></td><td>${esc(c.topic||'—')}</td><td>${esc(c.progress||'—')}</td><td>${esc(c.next||'—')}</td><td>${esc(c.headTeacher||'—')}</td><td>${esc(c.psychRep||'—')}</td></tr>`);
  $('homeProgress').innerHTML=table(['班级','当前课题','实际进度','下一步','班主任','心理委员'],homeRows);
...
async function loadCloud(){
  ...
  try{let r=await fetch(cfg.apiUrl,{cache:'no-store'});if(!r.ok)throw new Error('HTTP '+r.status);let j=await r.json();data=j;$('syncStatus').textContent='已同步 Google 数据';$('setup').style.display='none';render()}catch(e){$('syncStatus').textContent='云端同步失败';render()}
}
