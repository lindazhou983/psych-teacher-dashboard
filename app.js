...
function resourceItem(url,label){
  const ok=url&&/^https?:\/\//.test(String(url));
  return `<div style="display:flex;align-items:center;gap:8px;min-width:150px"><strong style="font-size:14px">${esc(label)}：</strong>${ok?`<a class="button" href="${esc(url)}" target="_blank" rel="noopener">打开${esc(label)}</a><span class="badge good">已上传</span>`:`<span class="muted">未上传</span>`}</div>`;
}
